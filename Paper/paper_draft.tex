...
\midrule
Drop Group 1 & [TBD] & [TBD] & [TBD] \\
Drop Group 2 & [TBD] & [TBD] & [TBD] \\
Drop Group 3 & [TBD] & [TBD] & [TBD] \\
Drop Group 4 & [TBD] & [TBD] & [TBD] \\
Drop Group 5 & [TBD] & [TBD] & [TBD] \\
Drop Group 6 & [TBD] & [TBD] & [TBD] \\
Drop Group 7 & [TBD] & [TBD] & [TBD] \\
Drop Group 8 & [TBD] & [TBD] & [TBD] \\
\midrule
\multicolumn{4}{l}{\textit{Range: [min]--[max]}} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Each row drops one merger group and re-estimates the baseline specification. Coefficients in the range 0.5--1.1 would suggest robustness; coefficients near zero or above 1.5 when dropping a single group would indicate that group is influential. MergerID and year FE included. Standard errors clustered by county. * p<0.10, ** p<0.05, *** p<0.01.
\end{tablenotes}
\end{threeparttable}
\end{table}

If all leave-one-out coefficients remain in a similar range (e.g., 0.5--1.1), the result is robust to the exclusion of any single merger group. Conversely, if dropping one group causes the coefficient to collapse toward zero or spike dramatically, that group would be driving the result, raising concerns about external validity.

\subsection{Summary of Evidence}

The pattern of results---significant in the baseline, attenuated but positive with county FE, no pre-trend in the placebo, and [robust/sensitive] to leave-one-out analysis---is consistent with fintech providing some mitigation, though the magnitude is uncertain. The most conservative interpretation is that the true effect lies between zero and the baseline estimate of 0.82, with considerable uncertainty.
