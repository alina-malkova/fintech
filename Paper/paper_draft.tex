...
A natural question is whether the fintech creditworthiness score predicts default equally well across different geographic contexts---particularly in banking deserts where traditional credit access is limited. Table \ref{tab:geo_heterogeneity} examines predictive power by banking desert status.

\begin{table}[H]
\centering
\caption{Fintech Score Predictive Power by Banking Desert Status}
\label{tab:geo_heterogeneity}
\begin{threeparttable}
\begin{tabular}{lcccc}
\toprule
Sample & Coefficient & Std. Error & AUC & N \\
\midrule
Full Sample & -2.164 & 0.063 & 0.926 & 5,746 \\
Non-Desert Counties & -2.213 & 0.065 & 0.928 & 5,577 \\
Banking Desert Counties & -1.803 & 0.359 & 0.911 & 98 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Coefficients from logistic regression of mortgage default on standardized fintech score. Banking desert = county with $<$1 branch per 10,000 population. AUC = Area Under ROC Curve. All coefficients significant at p $<$ 0.01.
\end{tablenotes}
\end{threeparttable}
\end{table}

The fintech creditworthiness score achieves strong predictive power in both contexts. In non-desert counties, the AUC is 0.928; in banking deserts, it is 0.911. While the coefficient is somewhat attenuated in banking deserts (-1.803 vs. -2.213), this difference is not statistically significant given the small desert sample (N = 98). The key finding is that the fintech-style score predicts default well even in underserved areas where traditional credit assessment may be most challenged.
...
The alternative creditworthiness analysis yields four main findings. First, a fintech-style credit score constructed from CAPS behavioral variables achieves exceptional predictive power for mortgage default (AUC = 0.926), validating that the characteristics fintech lenders claim to use for alternative underwriting are indeed predictive of loan performance. Second, financial resilience---access to emergency funds and home equity---is the key discriminating factor, with the bottom tercile showing a 52\% default rate versus 0\% in the top tercile. Third, the score predicts well across geographic contexts, with AUC of 0.928 in non-desert counties and 0.911 in banking deserts, suggesting that alternative creditworthiness assessment is effective even in underserved areas. Fourth, there is suggestive evidence that high-resilience individuals are more protected from branch closure effects (interaction t = -1.67, p = 0.098), consistent with the mechanism that alternative creditworthiness characteristics provide access to substitute credit sources.

These findings complement the county-level fintech analysis in several ways. While the county-level results are fragile to identification concerns, the individual-level creditworthiness validation does not depend on fintech penetration being exogenous---it simply asks whether behavioral characteristics predict outcomes. The strong predictive power of the fintech score (pseudo-R$^2$ = 0.457) provides confidence that these characteristics genuinely matter for loan performance, independent of identification concerns about county-level fintech adoption. The geographic heterogeneity analysis further demonstrates that the score's predictive power is not driven by differences across banking environments.
